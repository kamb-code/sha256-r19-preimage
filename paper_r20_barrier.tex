...
included.  Among the absorbed words, dependence is full-avalanche only in the
forward direction.  At $R{=}19$ every feedback edge passes through Maj/Ch
alone (at most $1.9$ bits of a constraint change per flipped bit); at
$R{=}20$ exactly one feedback edge is heavy, $a_4\to$C0 through $W_9$
($12.2$ bits), and the heavy dependency graph acquires its first cycle.  The
$R{=}19$ iteration works because it cuts its cycle at a mild edge; a heavy
cycle has no mild cut, and any $32$-bit cut of it is a random map.  The
...
admit a global table, show the assignment is forced up to a choice between
two frames, both of which close the heavy cycle, and record the resulting
$2^{64}$ cost for this attack family.  We record a second global
...
steps.  First an inventory of which absorptions admit a \emph{global} table,
correcting an omission in the version of this inventory we circulated
earlier.  Second a measurement of the dependency structure among the absorbed
words that explains, with one number per edge, why the $R{=}19$ iteration
architecture cannot be carried to $R{=}20$ in any frame.  Unlike the earlier
``irreducible cycle'' claim---which was wrong, and which we withdraw---neither
step is an argument about cycles alone: the $R{=}19$ assignment is also
cyclic and succeeds.  What matters is the \emph{weight} of the edges.
...
$1\le m\le7$ it occurs only inside a $T_2$, a $\Sig{1}$ or a $\Ch$ argument.
\end{proof}

The lemma is checked numerically by \texttt{edge\_weights.py}: for $k\le4$
at $R{=}20$ and three distinct increments each, $W_k$ moves by exactly
...
attack.  For $j=3$ it is C3$\to a_4$, which the inventory we circulated
earlier listed as absent; the corrected inventory is:
...
four.  Given the inventory there are exactly two ways to do it at $R{=}20$.
...
\]
Sweeping something other than $a_0$ does not help in either frame, because
$a_0$ enters C0 both inside $\sig{0}(a_1+gv(a_0))$ and additively through
$W_0$, and separating those requires a table indexed by $a_1$.  Both
...
\subsection{Edge Weights: the Heavy Cycle Appears at $R=20$}
\label{sec:heavy}
...
plus a constant, moves one or two bits.
...
heavy, $a_4\to$C0 at $12.2$ bits, and the heavy graph becomes the cycle
...
lookups, which is a random map on $2^{32}$ points; its fixed points cost
$2^{32}$ each to find, which is the classic sweep.  Frame A is worse, with
three heavy feedback edges through $a_{11}$.  This is the content of the
``dimensional'' explanation in \S\ref{sec:noniter}: the state cannot be held
at $32$ bits because no mild cut exists.  It is also why partial seeding of
$(a_2,a_3,a_4)$ never helped in our early $R{=}20$ experiments: the seed had
to be exact in $a_4$, because $a_4$ is the word on the heavy edge.
...
$\Sig{0}(a_4)$ from $W_9$, because no context word stands between them.  At
...
to a $2^{-15.8}$ bias).  The routes listed in earlier versions of this
section---a normal form for $\sig{0}(W_4)+W_3$ in one message word, freeing
the sweep variable through the $a_0$ absorber, a joint two-variable global
table---are all instances of cutting a heavy edge, and none has been found.
...
($w{=}4$, $41$ true solutions across its contexts), repeated after the
correction to the model's initial-state indexing (\S\ref{sec:redwidth}),
agrees: $2{,}560$ of $2{,}880$ orders find at least one solution, the median
finds $2$ and the best finds $11$, and the hand-chosen $R{=}19$-style order
(sweep $a_0$; $a_1\leftarrow$C0, $a_2\leftarrow$C1, $a_3\leftarrow$C2,
$a_{11}\leftarrow$C3) ranks $175$th with $5$.  The pre-correction run had
reported a median of $2$ of $35$; the conclusion is unchanged.

\section{Status}

$R=20$ is not solved.  The barrier is $2^{64}$ for this attack family, and its
...
unmeasured; taken at face value that is about $2^{62.2}$, an improvement and
not a solution.
...
enter shift-invariantly; no elimination order helps.  The heavy-cycle
