...
def make_ctx(rng, R):
    ...
    v = int(rng.integers(0, 1 << 32, dtype=np.uint64)) if VGEN else 0
    ctx = {4: v, 5: v}
...
def run(h, ctx, N, seed, label, R):
    ...
    assert e8 == M and e9v == M and a4 == a5, "family conditions violated"
    ...
    for b in range(max(N // B, 1)):
        ...
        sub = Maj(c(a4), A3, A2) == A3        # general-v form: Maj(v,a3,a2) == a3
        tot['sub'] += int(sub.sum())
...
grand = dict(surv=0, fp=0, sub=0, eps0=0, c3lo=0, ver=0, c3=0)
t_start = time.time()
for tseed in range(400, 400 + NT):
    ...
    t = run(h, make_ctx(rng, R), N, tseed * 17, f"sub t{tseed}", R)
    for k in grand: grand[k] += t[k]
fp = max(grand['fp'], 1)
print("")
print(f"GRAND TOTAL R={R} ({NT} targets, {NT*N:,} swept a0, submask family):")
print(f"  triangular solutions {grand['fp']:,} ({grand['fp']/max(grand['surv'],1):.4f} per survivor)")
print(f"  submask (eps==0)     {grand['sub']:,} -> {grand['sub']/fp:.4e} per solution "
      f"(predicted 1.0037e-04, enhancement over uniform {grand['sub']/fp/2.3283e-10:.3e}x)")
if R >= 20:
    print(f"  C3_lo==0 {grand['c3lo']:,} of {grand['sub']:,} -> {grand['c3lo']/max(grand['sub'],1):.3e} (uniform 1.526e-05)")
    print(f"  C3==0    {grand['c3']:,}")
print(f"  VERIFIED PREIMAGES   {grand['ver']:,}  -> {grand['ver']/(NT*N):.3e} per a0 "
      f"= 2^{np.log2(max(grand['ver'],1)/(NT*N)):.1f};  a0 per preimage = {(NT*N)/max(grand['ver'],1):,.0f}")
print(f"  [{time.time()-t_start:.0f}s]")
print("distinct verified messages:", len(SEEN), " total ver:", grand["ver"])
