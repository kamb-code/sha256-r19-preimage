...
for tseed in (11, 12, 13):
    ...
    print("=== target", h.hex()[:16], flush=True)
    base = {r: int(x) for r, x in zip(range(4, 11), rng.integers(0, 1 << 32, 7, dtype=np.uint64))}
    def with_e9(ctx):
        c2 = dict(ctx); a5, a6, a7, a8 = c2[5], c2[6], c2[7], c2[8]
        c2[9] = (M - a5 + T2(a8, a7, a6)) & M; return c2
    def with_a5(ctx):
        c2 = dict(ctx); c2[5] = c2[4]; return c2
    run_context(h, base, N, 5, "random")
    run_context(h, with_a5(base), N, 6, "a5=a4 only")
    run_context(h, with_e9(base), N, 7, "e9=-1 only")
    run_context(h, with_e9(with_a5(base)), N, 8, "both")
