...
GPUS = sys.argv[2].split(",") if len(sys.argv) > 2 else ["NVIDIA H100 80GB HBM3"]
LOG = os.path.join(S, f"r20_{TAG}_run.log")
STATUS_LOCAL = os.path.join(S, f"r20_{TAG}_status.json")
...
def create_pod():
    for gpu in GPU_CANDIDATES:
        body = {"name": f"sha20-{TAG}", "imageName": IMAGE, "gpuTypeIds": [gpu],
...
def main():
    log(f"R=20 submask-family run [{TAG}] on {GPUS}, cap {HOURS} h on the sweep, {WALL_CAP/3600:.1f} h on the pod")
    pod_id = create_pod()
    t_create = time.time()
    try:
        ...
        while True:
            ...
            if os.path.exists(GLOBAL_STOP):
                log("global stop seen (another pod hit); shutting down"); break
            if found or any(l.strip() == "EXITED" for l in lines) or elapsed > WALL_CAP:
                break
        for f in ("run.log", "status.json", "HIT.txt", "stdout.log"):
            scp(ip, port, f"/workspace/r20/{f}", os.path.join(S, f"r20_{TAG}_" + f), to_pod=False, timeout=300)
