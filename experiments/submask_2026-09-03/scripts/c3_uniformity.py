...
def run(h, ctx, N, seed, label, R):
    ...
    for b in range(max(N // B, 1)):
        ...
        if R >= 20 and idx.size:
            ...
            C3VALS.append(np.asarray(c3).copy())
...
for tseed in range(600, 640):
    ...
    run(h, make_ctx(rng, R), N, tseed * 19, f"c3 t{tseed}", R)
    if sum(v.size for v in C3VALS) > 200000: break
allc3 = np.concatenate(C3VALS)
n = allc3.size
print("")
print(f"C3 UNIFORMITY, conditioned on the submask condition: {n:,} samples")
for k in (4, 6, 8, 10, 12):
    bins = 1 << k
    cnt = np.bincount(np.asarray(allc3 & U32(bins - 1)), minlength=bins).astype(float)
    exp = n / bins
    chi2 = ((cnt - exp) ** 2 / exp).sum()
    dof = bins - 1
    z = (chi2 - dof) / np.sqrt(2 * dof)
    print(f"  low {k:>2} bits: chi2 = {chi2:9.1f}  dof {dof:>5}  z = {z:+6.2f}  "
          f"{'OK (uniform)' if abs(z) < 3 else 'DEVIATION'}")
    # also the high bits
    cnt = np.bincount(np.asarray(allc3 >> U32(32 - k)), minlength=bins).astype(float)
    chi2 = ((cnt - exp) ** 2 / exp).sum(); z = (chi2 - dof) / np.sqrt(2 * dof)
    print(f"  high {k:>2} bits: chi2 = {chi2:9.1f} dof {dof:>5}  z = {z:+6.2f}  "
          f"{'OK (uniform)' if abs(z) < 3 else 'DEVIATION'}")
print(f"  exact zeros: {(allc3 == Z).sum()} (expected {n * 2.3283e-10:.4f})")
print(f"  distinct values: {len(np.unique(allc3)):,} of {n:,} (collision check)")
