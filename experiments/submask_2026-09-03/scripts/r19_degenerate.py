...
print("target (19-round digest of a random 55-byte message):", h.hex(), flush=True)
N = int(sys.argv[2]) if len(sys.argv) > 2 else (1 << 22)

# (ii) random context
ctx_rand = {r: int(x) for r, x in zip(range(4, 11), rng.integers(0, 1 << 32, 7, dtype=np.uint64))}
run_context(h, ctx_rand, N, 5, "random ctx")

# (i) degenerate context: a5 = a4, a9 chosen so that e9 = 0xFFFFFFFF
ctx = dict(ctx_rand); ctx[5] = ctx[4]
a4, a5, a6, a7, a8 = ctx[4], ctx[5], ctx[6], ctx[7], ctx[8]
ctx[9] = (M - a5 + T2(a8, a7, a6)) & M          # e9 = a5 + a9 - T2(a8,a7,a6) = 0xFFFFFFFF
run_context(h, ctx, N, 6, "degenerate ctx a5=a4, e9=-1")
