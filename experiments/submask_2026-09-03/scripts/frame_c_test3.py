...
def run_instance(seed):
    ...
    ca, ok0 = calibration_A(N, seed=seed + 1000)
    cb, c3 = experiment_B(N, seed=seed + 2000)
    return ca, ok0, cb, c3

tot = dict(ca=0, ok0=0, cb=0, c3=0)
t0 = time.time()
for seed in range(100, 132):
    ca, ok0, cb, c3 = run_instance(seed)
    for k, v in zip(tot, (ca, ok0, cb, c3)): tot[k] += int(v)
    print(f"instance {seed}: R19-loop conv {ca:>3} of {ok0:,} survivors | frame C conv {cb:>3} of {N:,} pairs, C3 {c3}", flush=True)
n_inst = 32
print(f"TOTAL over {n_inst} instances: R19-loop {tot['ca']} / {tot['ok0']:,} survivors = {tot['ca']/max(tot['ok0'],1):.2e} per survivor "
      f"(paper ~1e-5); frame C {tot['cb']} / {n_inst*N:,} pairs = {tot['cb']/(n_inst*N):.2e} per pair; C3 passes {tot['c3']}  [{time.time()-t0:.0f}s]")
