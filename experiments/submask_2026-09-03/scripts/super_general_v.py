...
def make_ctx(rng, R, v=0):
    """a4=a5=v; a6,a7,a10(,a11) free; a8,a9 solve e8=e9=0xFFFFFFFF."""
    ctx = {4: v, 5: v}
...
def run(h, ctx, N, seed, label, R):
    ...
    for b in range(max(N // B, 1)):
        ...
        sub = Maj(c(a4), A3, A2) == A3      # general form; reduces to a3 & ~a2 == 0 at a4 = 0
        tot['sub'] += int(sub.sum())
...
R = 19
N = 1 << 21
print("Does the family work for a4 = a5 = v with general v?  (theory: eps = Maj(v,a3,a2) - a3)")
for vlabel, v in (("v=0x00000000", 0), ("v=0xffffffff", 0xffffffff),
                  ("v=0x6a09e667", 0x6a09e667), ("v=0xdeadbeef", 0xdeadbeef)):
    ...
    t = run(h, make_ctx(np.random.default_rng(888), R, v), N, 31, vlabel, R)
