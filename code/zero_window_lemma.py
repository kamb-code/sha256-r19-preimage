...
Uniqueness: among two-position input differentials with fixed ±δ,
  the full 8-round unconditional zero window is achieved only by (0,9)
  and its sign-reversal. Shifted pairs are checked below and fail in
  general because nonlinear σ-terms enter the proposed window.
""")

# Verify shifted pairs do not accidentally create the same unconditional window.
print("Checking shifted pairs (i, i+9):")
...
for i in range(7):
    ...
    expected = (i == 0)
    status = "as expected" if win_ok == expected else "UNEXPECTED"
    print(f"  Pair ({i},{j}): zero window W[{win_start}..{win_start+7}] = {win_ok} ({status}),  ΔW[{j+16}]={'-δ' if echo==NEG_DELTA else hex(echo)}")
