...
def feed_forward(S):
    return [(H0[i] + S[i]) & M for i in range(8)]
...
H = feed_forward(S)
Hp = feed_forward(Sp)
print(f"\nRaw post-round state(IV, M)  = {[hex(x) for x in S]}")
print(f"Raw post-round state(IV, M') = {[hex(x) for x in Sp]}")
print(f"SHA-256_compress output(IV, M)  = {[hex(x) for x in H]}")
print(f"SHA-256_compress output(IV, M') = {[hex(x) for x in Hp]}")
print(f"\nΔState[64] = output(IV, M') - output(IV, M):")
for i in range(8):
...
print(f"  The 5th output word is: 0x{H[5]:08x} = 0x{Hp[5]:08x}")
print(f"  Raw post-round word before IV feed-forward: 0x{S[5]:08x} = 0x{Sp[5]:08x}")
