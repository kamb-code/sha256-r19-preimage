...
$\Maj(a_4,a_3,a_2)-a_3 \mmod 2^{32}$, whose vanishing is a bitwise condition of
...
$1/(c^3(3/4)^{32})=39{,}124$: about 12 milliseconds of one CPU core.  We
...
NVIDIA A100.  We report two computed and verified 20-round preimages, one for
the all-ones digest, found after 16~h on one A100 for about \$26.  To our knowledge, and after the search described in
...
published.  We also show that, for this construction, 21 rounds costs the full
additional $2^{64}$: the fourth and fifth constraints are independent, and the dependency
that blocks a further collapse passes through $\Sig0$ of the unknown itself,
...
has been executed, and the cheapest figure anywhere in that line is $2^{240}$
...
We choose the context instead of drawing it at random.  Four conditions,
$a_4=a_5$ together with $e_8=e_9=\hex{0xFFFFFFFF}$ (each imposed by solving one
round equation for one context word), have two consequences
...
\item At 19 rounds the cost per preimage is about $2^{15.25}$ swept $a_0$,
  ...
  the pooled measured value is $39{,}123$.  One CPU core produces a preimage
...
\item At 20 rounds the fourth schedule constraint is an exact 32-bit filter
  that nothing in the construction can bias (Section~\ref{sec:r20}).  With
  ...
  all-ones digest, after 16.2~h on one A100, about \$26.  To our knowledge, and
...
$a_0$ each, i.e.\ $2^{38.3}$ swept $a_0$ per preimage.\footnote{The pooled
campaign estimate of~\cite{Basra2026R19} mixes a screened arm with a control
arm; the population-weighted point estimate for a random context is one
preimage per 133 attempts, with a wide interval.  Either figure is what this
paper improves on by about $2^{23}$; the distinction does not affect the
comparison.}
...
\label{cor:yield}
With a single-root table of image fraction $c=0.633673$, each swept $a_0$
yields $c^3=0.2545$ triangular solutions, of which a fraction $(3/4)^{32}$
satisfy~\eqref{eq:eps}.  Every such triple is a 19-round preimage.  The
...
The base attack's iteration converges at about $2^{-18}$ per attempt rather
than a random map's $2^{-29}$ because the feedback into $C_1$ passes through a
bitwise select that leaves only about $2^{22}$ effective states; the family
...
already stored) raises the per-lookup coverage to $0.8979$ and $0.9774$, and
the triangular solutions per swept $a_0$ to $0.7244$ and $0.9337$; measured
values are $0.7235$ and $0.934$.  Each table is 16~GB; three fit an 80~GB GPU.
The gain in wall-clock is smaller than the gain in solutions, since the extra
branches cost lookups: about $1.25\times$ on a CPU and about $2\times$ on a
memory-bound GPU for two roots.  One index in $2^{32}$ is lost to the sentinel
...
GPU, and sustains $8.6$--$9.0\times10^{8}$ in production including host
overhead, i.e.\ $4.7$--$5.0$~s per context of $2^{32}$ swept $a_0$.  Averaged
...
\midrule
4 random targets & $16{,}777{,}216$ & $4{,}270{,}445$ & $0.4015$ & 395 & $42{,}474$ \\
...
$^{a}$ Twenty-four contexts, twenty-four targets and twelve values of $v$, run
by an independent reviewer from a fresh implementation; 6{,}915 of 6{,}915
collapsed-condition candidates verified as preimages.  Derived value:
$39{,}124$.
\end{minipage}
\end{table}

Table~\ref{tab:r19} summarises the 19-round measurements.  The 64-target run
took 151.7~s in one numpy process (about 1.35 cores); pinned to a single core
the rate is $3.4\times10^{6}$ swept $a_0$ per core-second
...
1.0045\times10^{-4}$; an absolute yield audit, observed preimages divided by the
prediction of Corollary~\ref{cor:yield}, gives $0.9986\pm0.0029$, so any
...
so the speedup is $2^{23}\pm1.5$ bits.  Against Zaikin~\cite{Zaikin2026} the
...
\end{tabular}
\begin{minipage}{0.95\textwidth}\footnotesize\vspace{2pt}
$^{b}$ Counts are in the logs; twenty sample blocks with their targets are in
the repository.  $^{a}$ About 3{,}560 core-hours; obtained from an intermediate problem whose
...
the 19-round instance found nothing in seven days on 192 cores.
\end{minipage}
\end{table}
...
\begin{equation}
c_3 \;=\; \sig0(W_4) + W_3 - K_3 ,\qquad
K_3 = W_{19}-\sig1(W_{17})-W_{12},
\label{eq:c3}
\end{equation}
which must vanish, where $W_3,W_4$ depend on $a_0,\dots,a_4$ and $K_3$ on the
...
\item The GPU campaign's own counters
  ...
  (expected $2{,}685$), $z=-0.85$; the worst single step ($25\to26$, 78 to 28,
  one-sided binomial $p=0.008$) is $p\approx0.10$ after correcting for the
  twelve steps examined, and the two A100 pods give $z=+1.01$ and $z=+1.35$ on
  the same steps.  Including the step $19\to20$, on which the H100 alone is
...
\item A dedicated test on the all-ones digest itself, against random digests
  ...
  $3.95\times10^{6}$ candidates in each arm): no deficit appears at any depth
  in either arm.  The all-ones arm runs mildly high between 18 and 20 bits
  (at 20 bits, 11 observed against 3.8 expected) and returns to expectation
...
$K_3$; the word $v=a_4$ enters $W_4$ additively before $\sig0$, and the
context constants also determine which $(a_1,a_2,a_3)$ the three lookups
return.
Writing $S=\sig0(W_4)+W_3$, a sum modulo $2^{32}$, its characteristic
function factorises, $\varphi_{c_3}(\beta)=\varphi_{W_3}(\beta)\,
\varphi_{\sig0(W_4)}(\beta)\,e^{-2\pi i\beta K_3/2^{32}}$.  The
largest-root table policy gives $W_3$ a strong bias
($|\varphi_{W_3}(1)|=0.155$, a $255\sigma$ departure from uniform), and it is
annihilated because $|\varphi_{\sig0(W_4)}(1)|=0.025$ sits at its own noise
floor; the product predicts the observed $|\varphi_{c_3}(1)|=0.026$, which is
noise.  Hence, at our sample size, no table representative policy, even one
driving $|\varphi_{W_3}|$ to~1, moves $|\varphi_{c_3}(1)|$ above $0.025$,
worth at most $9\times10^{-4}$ bits; and ``some contexts are cheaper'' is identically
the statement ``$S$ is non-uniform'', which an $8.4\times10^{6}$-frequency
Fourier scan with a validated positive control rejects.  A policy that stores
...
($<1.6\times10^{-4}$ bits over 15{,}360 tests); $17.3\times10^{6}$ random
...
\$24 at \$1.59 per hour; on an H100 the same kernel would take about 8~h at a
higher hourly rate.  Being a memoryless search, the time to the first
...
A100 & PyTorch & random digest & $\approx1{,}852$ & $7.96\times10^{12}$ & 0.174 & none \\
A100 & PyTorch & random digest & $\approx1{,}850$ & $7.9\times10^{12}$ & 0.17 & none \\
...
Table~\ref{tab:campaign} lists every GPU run.  Two preimages were found from a
...
inverted at 19 rounds and chose, in his words, so that it could not be said
that a random input--output pair had been picked; no 20-round preimage of it
was known to exist:
\begin{center}\ttfamily\small
...
Because no 20-round preimage existed before this work, the fourth-constraint
code path had never been exercised on a genuine input, and an error in it
...
rounds (measured; 40 random instances each; \texttt{logs/frame\_search.log}).}
\label{tab:frames}
\begin{tabular}{@{}lccc@{}}
\toprule
...
$\Maj(a_5,a_4,a_3)$.  Either way a non-maskable path remains.  A symbolic
search over 1{,}485 legal context-condition families confirmed that none
clears it.  The collapse of Proposition~\ref{prop:collapse} survives the
...
On 214{,}583 freshly generated 21-round candidates satisfying everything up to
the collapsed condition, the residuals of $C_3$ (at $W_{19}$) and $C_4$ (at
$W_{20}$) are each uniform and their mutual information is below $10^{-5}$
bits.  Twenty-one rounds therefore costs this construction the full $2^{64}$
above nineteen, about $2^{77}$ swept $a_0$ with three-root tables ($2^{79}$
with one), beyond reach.
...
Proposition~\ref{prop:collapse}.  The joint entropy of $(a_2,a_3)$ is
$50.7189$ bits against the predicted $32\log_2 3=50.7188$, with the surviving
pairs uniform on the allowed set, so the filter is information-theoretically
tight; the GF(2) rank deficit is exactly the three carry-free least-significant
bit relations the construction implies; and no other test departed from its
null.  Fifteen independent analyses proposed eight candidate levers and every
one was refuted on re-examination.  One methodological note: a corpus that
...
all-zero digest, 19 only in a weakened form (fewer than 32 bits of the digest
fixed), and 19-round collisions, by Davydov et~al.~\cite{Davydov2025}; 19 rounds for the
...
the SAT, CP and AI venues where the practical line lives, Zaikin's
repositories, and Russian- and Chinese-language sources, from four
independent starting points.  No computed preimage of the SHA-256 compression
function at 20 or more rounds was found, in any model, weakened or not.
...
chaining value: weakened on three axes at once.  Bouam et~al.~\cite{Bouam2021} exhibit three
messages whose \emph{full} SHA-256 digests agree, under exclusive-or, on their
first 128 bits, a brute-force computation of about $8.7\times10^{22}$
operations; that is a collision-type result on half the output, not a preimage
...
\begin{itemize}
\item \cite{Basra2026R19} states about $2^{38.3}$ swept $a_0$ and about two
  GPU-hours per 19-round preimage with random contexts.  Those figures remain
...
\item The screening campaign of~\cite{Basra2026R19} reports a $3.41\times$
  ...
  duplicates are $0.03\%$, and identity-level deduplication can move it only
...
Choosing four context words rather than drawing them at random removes the
...
compression function by about $2^{23}$, to milliseconds on one CPU core, with
the measured rate matching the derived one to four digits.  The same
...
at 19 rounds on 192 cores.  No computed preimage beyond 19 rounds appears in
the literature.  Twenty-one rounds costs the full additional $2^{64}$ for a
reason we can state exactly: the surviving dependency passes through $\Sig0$
of the unknown, which no context word can saturate.  Full SHA-256 is
untouched.
...
passes described in Appendix~\ref{app:repro}.  All results were verified by
code that shares nothing with the attack implementation and can be
...
\newblock \emph{Constraints}, published online 23 January 2026 (received
  28 March 2025).  DOI 10.1007/s10601-025-09383-0.
