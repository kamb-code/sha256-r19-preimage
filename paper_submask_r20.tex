...
\paragraph{The obstruction is a single edge.}
The statement can be made sharper than Table~\ref{tab:frames}.  Two general
facts hold at 20 rounds (\path{analysis/elimination_search/}).  First, an
...
both are one-input functions of $a_k$ itself, so no context word can saturate
them.  Second, $a_1$, $a_2$ and $a_3$ have forced homes: in every one of
...
and $a_3$ only by $C_2$.  Hence a fourth absorbed word must be light in
...
$a_4\to C_0$ edge; the frozen residual is a random 32-bit map
($P(\text{residual}=0)=2\cdot2^{-32}$, no affine output bit)
(\path{analysis/elimination_search/falsify_a5/}).  The obstruction to a fourth
absorption at 20 rounds is therefore not a web of dependencies but this single
edge, and it is heavy for the same reason as the $a_4\to C_0$ edge of
\cite{Basra2026R20}: $\Sig0$ of the unknown itself.  We state this for this
construction and its menu of context conditions; it is not a claim about
attacks that are not table-based.
