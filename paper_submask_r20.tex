...
positions sum to $0.001$ bits, so within the resolution of the corpus the
surviving pairs are uniform on the allowed set and the filter leaves no
further structure; the GF(2) rank deficit is exactly the three carry-free least-significant
...
all-zero digest, 19 only in a weakened form (at most 24 of the 256 digest bits fixed, to
zero), and 19-round collisions, by Davydov et~al.~\cite{Davydov2025}; 19 rounds for the
...
function beyond 19 rounds had previously been published.  For this construction, twenty-one rounds costs the full additional
$2^{64}$, for a reason we can state exactly: the surviving dependency passes
