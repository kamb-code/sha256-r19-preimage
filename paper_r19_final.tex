...
rather than search-based, that reaches an arbitrary digest in about half an
hour of one GPU, with complete artefacts.  A companion note explains why the
...
$^{b}$\,Parameterized Kissat inside Cube-and-Conquer, via an intermediate
problem between rounds 18 and 19 (see text).
\end{minipage}
\end{table}
...
in seven days on 192 cores; the preimage came from the intermediate problem
between rounds 18 and 19 that keeps 29 of the 32 bits of the last message
word, whose solutions are full 19-round preimages with probability $1/8$,
...
rounds.  Seven off-the-shelf parallel SAT solvers found nothing.  The
attack model is the same as ours.  What the present work adds is a
different method with a different cost profile: after a one-time $16$\,GB
precomputation, each attempt is one $2^{32}$ sweep of about $25$\,s on a
single GPU, about one attempt in eighty succeeded in the campaign of
\S\ref{sec:screening}, so a preimage of an arbitrary digest costs of the
order of half an hour of one H100, and six preimages of random digests are
published together with the code that found them.  We do not convert
