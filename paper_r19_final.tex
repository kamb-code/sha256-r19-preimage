...
reusable 16-GiB table for $u\mapsto\sigma_0(u)-u$.  Two constraints form a
fixed-point iteration; an exact final test checks the provisional treatment
of $W_9$.  A complete sweep took about 25 seconds on one NVIDIA H100 with one
...
an intermediate low-16-bit counter by a trajectory-counted factor 3.41
(identity-level bounds 2.42--3.44); its effect on final success remains
unresolved.  With only three campaign events, the data do not determine a
mean runtime.  A newer chosen-context paper supersedes these performance
figures and gives computed 20-round examples.  Neither paper
...
Let $F_R$ denote the eight-word state after the first $R$ SHA-256 rounds,
before feed-forward, and let $\mathrm{IV}$ be the standard initial value.
Given a 256-bit digest $h$, a \emph{preimage} in this paper is a 512-bit block
$(W_0,\ldots,W_{15})$ with
\[
  h \;=\; \mathrm{IV} + F_R(\mathrm{IV},\,W_0,\ldots,W_{15}) .
\]
...
55-byte message the padding would constrain parts of
$W_{13},W_{14},W_{15}$.  Although the repository contains an exploratory
closed-form way to impose those word values, no padded preimage was obtained;
we make no padded-hash claim.
...
The practical preimage frontier belongs to SAT-based cryptanalysis, which Mironov and Zhang~\cite{MironovZhang2006} applied early to hash functions, in the collision setting.  Davydov, Pikh\-tov\-ni\-kov,
...
function, standard IV, no padding, feed-forward retained.  The 19-round
...
compression function.  After a one-time 16-GiB precomputation, each attempt
is a single $2^{32}$ sweep on one GPU: about 25 seconds on the recorded H100
with one seed chain, or 89 seconds with the four chains used in the campaign.
The campaign was stratified.  Its selected arm produced $3/120$ successes and
its control arm $0/120$; the pooled $3/240$ rate describes that campaign but
not a uniform draw from the context population.  A plug-in
population-weighted point estimate,
$0.3(3/120)+0.7(0/120)=7.5\times10^{-3}$, corresponds to about 3.3 GPU-hours,
but only three total events make this estimate highly uncertain.  We therefore
report observed work and rates rather than a precise mean runtime.  The
ingredients are:
\begin{enumerate}
  ...
  \item seven forward-verified preimages: six of digests of random
    messages, three from dedicated runs and three from a preregistered
    240-attempt campaign that records arm-specific success counts, and
  ...
  \item a measured, preregistered account of how much a cheap screening
    ...
    (\S\ref{sec:screening}).
\end{enumerate}
A companion note~\cite{Basra2026R20} records finite negative results for a
direct 20-round extension.  A later chosen-context construction
\cite{Basra2026Submask} supersedes this paper's performance figures and gives
two computed 20-round examples; see the note added at the end.  Appendix~\ref{app:ninestep} records an exact
...
\end{align}
Here $W_{16}, W_{17}, W_{18}$ are recovered from the target and the context via the
...
When multiple $u$ share the same $v$, the deterministic on-disk table stores
the largest such $u$.  The GPU build used for the runs
(\texttt{build\_table} in \texttt{h100\_extended.py}) writes roots in
ascending chunks with a CUDA scatter whose winner at repeated indices PyTorch
does not specify; it stores a valid root but not necessarily the largest.
Experiment E3 compares smallest- and largest-root policies on one matched
sample and detects no resolved difference there; it does not establish policy
invariance in general.  On disk the table is an \texttt{int32} array with
sentinel $-1$; the value $v=\mathtt{0x20000000}$, whose only root is
$u=\mathtt{0xFFFFFFFF}$, is consequently treated as a miss.  The GPU sentinel
is the \texttt{int32} minimum and can analogously obscure that bit pattern if
it is the selected representative.  Either format can lose at most one stored
domain representative, which is negligible for the reported rates.
\end{definition}
...
takes under 2 seconds on the recorded H100 and requires 16\,GiB of GPU memory.

\paragraph{Coverage.}
The function $f(u) = \sig{0}(u) - u \bmod 2^{32}$ is not injective.
An exhaustive construction finds $2{,}721{,}603{,}628$ of the $2^{32}$ possible values
...
For the context rounds $r = 7,\ldots,11$, compute $T_2^{(r)}$ and
$T_1^{(r)}$, and for $r=8,\ldots,11$ also $e_r = a_{r-4} + T_1^{(r)}$,
from~\eqref{eq:T1}--\eqref{eq:e} using the context $a_4,\ldots,a_{10}$
together with $a_{11}$ from the backward chain.  ($e_7 = a_3 + T_1^{(7)}$
depends on the unknown $a_3$ and is not a context constant.)
Define:
\begin{align}
...
\begin{align}
  ...
  F_{\mathrm{C2}}(a_2) &= W_{18}^{\mathrm{bc}} - \sig{1}(W_{16}^*)
...
\[
  (a_2,a_3) \;\mapsto\; (a_2',a_3'),\qquad
  a_2' = \mathcal{T}[F_{\mathrm{C1}}(a_3)] - F_{12},\qquad
  a_3' = \mathcal{T}[F_{\mathrm{C2}}(a_2')] - F_{23}(a_0,a_1,a_2'),
\]
in which the C2 lookup uses the freshly updated $a_2'$ (Gauss--Seidel order, as implemented in \texttt{h100\_extended.py}); a step in which either lookup misses leaves the pair unchanged.
...
Pre-computing $K$ initial pairs $(a_2,a_3)$ that satisfy the low-16-bit constraint $W_9^{\mathrm{conv}}(a_2,a_3) \bmod 2^{16} = \hat{W}_9 \bmod 2^{16}$, i.e.\ $\varepsilon(a_2,a_3) \equiv 0 \pmod{2^{16}}$, was measured in an early pilot to yield about $3.3\times$ more $W_9$ lo-pass events per unit of compute than the single $(0,0)$ start; no per-context lo-pass difference is visible between the $K_{\mathrm{seeds}}{=}0$ and $K_{\mathrm{seeds}}{=}4$ datasets of Table~\ref{tab:stats}.
...
$\mu$ (mixed campaign arms, 240 ctx) & $1.25\times10^{-2}$\ \ Garwood CI $[2.6{\cdot}10^{-3},3.7{\cdot}10^{-2}]$ \\
\quad screened arm (120 ctx, $N_{LH}=3$) & $2.50\times10^{-2}$ \\
\quad control arm (120 ctx, $N_{LH}=0$) & $<3.1\times10^{-2}$ (Garwood, 95\%) \\
...
low half.  Experiment \textbf{E2}, a separately written CPU reimplementation
...
To test sensitivity to the lookup table's representative policy, a third
experiment (\textbf{E3}) repeated the measurement on byte-identical
...
observed.  This matched sample detects no resolved policy effect, but does not
establish that all representative policies are equivalent.  E3 used its own contexts and sample, which is why its lo figure
...
\[
  ...
  \qquad 95\%\ \text{Garwood interval}\
  [2.58\times10^{-3},\ 3.65\times10^{-2}].
\]
Split by arm, the screened half gave $3/120 = 2.50\times10^{-2}$ and the
control half $0/120$, i.e.\ $\mu < 3.1\times10^{-2}$ (upper limit of the
two-sided 95\% Garwood interval under a Poisson count model).  Two cautions
apply.  The pooled figure mixes the two arms and the
...
$14$, so this bounds the mixed-arm rate rather than pinning it.  The result is
numerically compatible with the $6\times10^{-3}$--$1.3\times10^{-2}$ band
projected before the campaign, but the stratified design and three events do
not support a stronger validation claim.
...
per-context count by the number of chains.  The E1 fixed-point figures above
are globally deduplicated on $(a_0,a_1,a_2,a_3)$ within a context.  The
campaign's downstream counters are trajectory-counted and treated separately
in Section~\ref{sec:screening}.

\paragraph{What the three dedicated finds do and do not establish.}
The dedicated runs found preimages in their 4th, 21st and 35th contexts
(solver indices 3, 20 and 34), three successes in 60 attempted contexts.  We
present these as demonstrations on separate targets, not as a runtime
estimator: three observations cannot determine the mean, and context
heterogeneity means that the mean number of matches per context need not equal
the probability of at least one match.  Their recorded solving times were
81.7, 511.1 and 871.6 seconds, for 1{,}464 seconds combined with the single
$(0,0)$ seed.  Across 57 completed sweeps and the three partial winning
sweeps, the average was about 25.7 seconds per full-sweep equivalent.  The
four-seed campaign measured 89 seconds per context; these are separate
configurations and both timings are retained.

\subsection{Context Screening}
\label{sec:screening}

The clustering just hypothesised is visible and predictive for the
intermediate fixed-point counter.  The
...
paired campaign (\path{campaign_screening.py}, seed 20260810).  The run
manifest in \path{data_campaign_screening.json} records the command, seed
and software versions, but the code commit is ``unknown'', target digests are
truncated, and the random source messages were not saved.  The aggregate rows
and the three hit blocks are preserved, but the campaign cannot be replayed
byte for byte from the archive.  We use it rather than assuming proportionality.

\emph{Design, fixed before the run.}  40 independently generated targets; 10 contexts
...
The selected arm represents three of ten generated contexts per target.  A
plug-in population estimate using the two observed strata is therefore
$0.3(3/120)+0.7(0/120)=7.5\times10^{-3}$ per full context, corresponding to
about 3.3 hours at the recorded 89-second four-seed rate.  With no control-arm
events and only three overall, this is a descriptive point estimate with very
wide uncertainty, not an established mean.

\emph{Summary of the claim.}  Screening costs $1/4096$ of a context's budget
and delivers a trajectory-counted $3.41\times$ (target-blocked bootstrap 95\%
interval $[2.13, 5.82]$ before identity-level correction) improvement in
lo-pass yield per attacked context; the available row aggregates bound the
identity-deduplicated point within $[2.42,3.44]$.  It is \textbf{not} the $12.99\times$
...
elements.  The 16\,GiB $\sigma_0$-difference table is held in GPU memory
throughout the
...
with $K_{\mathrm{seeds}}{=}4$.  Only the target digest was supplied to the solver; the six generated targets are 19-round digests of random 55-byte messages, so a preimage was known to exist, whereas the all-ones digest is the structured target of~\cite{Zaikin2026}, not known in advance to have one.  All values are hexadecimal.  Context numbers are the solver's zero-based indices.  $^\dagger$Found 6\,s into the fourth context (index 3).  $^\ddagger$Found 20\,s into the 84th context; 7{,}373\,s is the
whole run.}
\label{tab:preimages}
\footnotesize
\begin{tabular}{@{}lll@{}}
...
P7 & \texttt{ffffffff ffffffff ffffffff ffffffff} & 7{,}373\,s \\
   & \texttt{ffffffff ffffffff ffffffff ffffffff} & ctx~83$^\ddagger$ \\
...
Each row of Table~\ref{tab:preimages} was forward-verified by
the following procedure:
\begin{enumerate}
  \item Parse the 16 words $W_0,\ldots,W_{15}$ as a 512-bit block.
  \item Run the SHA-256 compression function for 19 rounds from the
    standard IV, with the standard message schedule.
...
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.0cm}c
  ...
  >{\raggedright\arraybackslash}p{3.5cm}c@{}}
\toprule
...
\midrule
\textbf{This work} & \textbf{19} & mixed campaign: $3/240$ at $89$\,s per attempt$^{c}$ & generated digests (six); all-ones & \textbf{seven} \\
\bottomrule
\end{tabular}
...
$^{b}$\,One successful run.  Parameterized Kissat inside Cube-and-Conquer,
via an intermediate problem between rounds 18 and 19 whose solutions are
19-round preimages with probability $1/8$ by his account; that acceptance
probability alone does not determine a mean runtime.\quad
$^{c}$\,Screened arm $3/120$, control arm $0/120$.  Pooling gives about two
GPU-hours per observed event, but is not a random-context estimate; the
population-weighted plug-in estimate is about 3.3 hours and is highly
uncertain.  The six generated targets are image-derived.
\end{minipage}
\end{table}
...
rounds.  Eight off-the-shelf parallel SAT solvers, seven from the SAT
Competition 2024 and ALIAS, found nothing.  Zaikin's 18~h~33~min is one
successful duration; neither it nor the stated $1/8$ acceptance probability
alone establishes a repeated-run mean.  The attack model is the same as ours
(his published preimage, Table~7 of~\cite{Zaikin2026}, verifies under
\texttt{verify\_r19.py} at 19 rounds).  What the present work adds is a
different method and seven published witnesses.  After a reusable 16-GiB
precomputation, one $2^{32}$ attempt took about 25 seconds with a single seed
chain and 89 seconds with the four chains used in the campaign.  That campaign
gave $3/120$ successes in its selected arm and $0/120$ in its control arm.
The pooled two-hour figure and the population-weighted 3.3-hour plug-in figure
describe that small stratified sample; neither is a precise mean.  We do not
convert between GPU-hours and CPU core-hours.

\paragraph{Head-to-head on the same target.}
After reading~\cite{Zaikin2026} we ran the corrected solver on its fixed
all-ones target with the four-seed campaign configuration.  It found P7
20 seconds into its 84th context, after 7{,}373 seconds (122.9 minutes) of
solving on one H100.  P7 differs from Zaikin's Table~7 block and is therefore
a second preimage of that reduced-round target.  The complete run is archived
in \texttt{data\_ones\_target\_run.txt}; an earlier aborted debugging run is
...
sweep of table lookups per attempt.  Six preimages of generated digests and
one of the all-ones digest attacked by Zaikin~\cite{Zaikin2026} were found
and forward-verified.  A preregistered stratified campaign then recorded
three successes in 120 screened attempts and none in 120 controls.  The
pooled value $3/240$ is a description of that campaign, not a
random-context success rate; the population-weighted plug-in estimate is
about 3.3 GPU-hours per event and has substantial sampling uncertainty.  A
cheap screening pass improved the trajectory-counted intermediate yield by
$3.41\times$ (identity-level deduplication bounds it between $2.42\times$
and $3.44\times$).  The campaign does not establish that this intermediate
improvement reduces final-preimage time.

The later context-shaped construction~\cite{Basra2026Submask} removes the
fixed-point iteration and supersedes this paper's performance figures.  It
also reports computed 20-round examples under an explicitly conditional
model for the extra schedule constraint.  The earlier barrier
note~\cite{Basra2026R20} records why the finite representations and update
orders tested there did not absorb that constraint; it is not an
impossibility result for other decompositions.

\paragraph{Artefacts.}
Solver, table construction, campaign driver, raw campaign data and an
standalone verifier are available at
...
difference, the expected minimum Hamming weight of a random 256-bit output difference over $2^{32}$ independent trials is $78$ (median $78$), and over the $2^{37}$--$2^{39}$ evaluations used by the multi-block searches it is $74$--$73$; a minimum of $72$ or below occurs by chance with probability $0.11$--$0.38$ over that many independent trials, and a directed descent does at least as well.  The
...
%-----------------------------------------------------------------------
\paragraph{Note added, September 2026.}  After this paper was written, a
context-shaped variant~\cite{Basra2026Submask} imposed $a_4=a_5$ and, for
the derived words, $e_8=e_9=\mathtt{0xFFFFFFFF}$.  Those conditions remove
the fixed-point iteration and collapse the consistency check to a bitwise
condition.  The observed 19-round cost falls to about $2^{15.25}$ swept
$a_0$ values.  At 20 rounds a fourth exact equality remains; treating its
measured residual as uniform at exact zero gives the paper's conditional
estimate of about $2^{45.4}$ swept values and about 15 A100-hours.  Two
computed 20-round examples are reported there, including one for the
all-ones digest.  That paper supersedes the cost estimates here.  The
campaign measurements and screening result in this paper remain evidence
about the original construction, but the small stratified campaign does not
provide a precise random-context mean.

\section*{Acknowledgements and disclosure}
The derivations, code, experiments and original text were developed with
substantial assistance from Anthropic's Claude, used through Claude Code under
the author's direction.  OpenAI Codex assisted with this revision by auditing
claims against the archived records, checking references, editing the
manuscript and rebuilding the publication files.  The repository also records
separate AI-agent verification passes.  Three of those passes implemented the
reduced-round compression function separately from the specification and
forward-checked supplied witnesses; other passes reviewed shared source,
manifests or logs.  They are automated checks, not human peer review.  The
author selected the claims and remains responsible for the manuscript.

\begin{thebibliography}{10}

\bibitem{Basra2026Submask}
K.~S.~Basra.
\newblock Context shaping for 19- and 20-round {SHA-256} compression
  preimages.
