...
rather than search-based, that reaches an arbitrary digest in about two
hours of one GPU in expectation, did so for his target in $123$ minutes,
and comes with complete artefacts.  A companion note explains why the
...
about one attempt in eighty succeeded, so a preimage of an arbitrary digest
costs about two GPU-hours in expectation.  The ingredients are:
\begin{enumerate}
...
with $K_{\mathrm{seeds}}{=}4$.  Only the target digest was supplied to the
solver.  All values are hexadecimal.  $^\dagger$Found inside the third
context.  $^\ddagger$Found 20\,s into the 84th context; 7{,}373\,s is the
whole run.}
\label{tab:preimages}
\footnotesize
\begin{tabular}{@{}lll@{}}
...
\midrule
\textbf{This work} & \textbf{19} & ${\approx}2^{32}$ per attempt, ${\approx}80$ attempts & random; all-ones & \textbf{seven} \\
\bottomrule
\end{tabular}
...
rounds.  Seven off-the-shelf parallel SAT solvers found nothing.  Since,
...
is measured over $338$ attempts with an interval.  The attack model is the
same as ours.  What the present work adds is a
...
single GPU with four seed chains, about one attempt in eighty succeeded in
the campaign of \S\ref{sec:screening}, so a preimage of an arbitrary digest
costs about two hours of one H100 in expectation, and six preimages of
random digests are published together with the code that found them.  We do not convert
